...
class subtitle_manager {
    ...
    public static function queue_source_generation(\stdClass $videotracker, \stdClass $cm): \stdClass {
        ...
        $unavailablereason = self::get_source_generation_unavailable_reason();
        if ($unavailablereason !== '') {
            throw new \RuntimeException(get_string($unavailablereason, 'videotracker'));
        }
    ...
    /**
     * Import a manually uploaded source WebVTT file.
     *
     * @param \stdClass $videotracker Activity record.
     * @param \stdClass $cm Course module record.
     * @param string $content Uploaded WebVTT content.
     * @param string $langcode Source language code.
     * @return \stdClass
     */
    public static function import_source_vtt(
        \stdClass $videotracker,
        \stdClass $cm,
        string $content,
        string $langcode
    ): \stdClass {
        ...
        $langcode = self::normalise_language_code($langcode);
        if ($langcode === '') {
            throw new \RuntimeException(get_string('subtitleerrorinvalidsourcelanguage', 'videotracker'));
        }

        $cues = self::parse_vtt($content);
        if (empty($cues)) {
            throw new \RuntimeException(get_string('subtitleerrorinvalidsourceupload', 'videotracker'));
        }

        $vtt = self::compose_vtt($cues);
        $hash = sha1($vtt);
        $time = time();
        ...
        if ($track) {
            ...
            $track->langlabel = $langlabel;
            $track->status = self::STATUS_READY;
            $track->basesourcehash = $hash;
            $track->currenthash = $hash;
            $track->openaimodel = '';
            $track->lasterror = '';
            $track->timemodified = $time;
            $DB->update_record('videotracker_subtitles', $track);
    ...
    /**
     * Return why automatic source generation is unavailable, if applicable.
     *
     * @return string Empty string when automatic generation is available.
     */
    public static function get_source_generation_unavailable_reason(): string {
        if (!function_exists('exec')) {
            return 'subtitleerrorexecdisabled';
        }

        $config = self::get_processing_config();
        if (trim((string) ($config['ffmpegpath'] ?? '')) === '') {
            return 'subtitleerrorffmpegmissing';
        }
