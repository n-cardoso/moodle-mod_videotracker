...
if (!\mod_videotracker\local\license_enforcer::subtitles_enabled()) {
    $systemcontext = \context_system::instance();
    $settingsurl = has_capability('moodle/site:config', $systemcontext)
    ...
    $PAGE->set_title(get_string('licensedemosubtitlestitle', 'videotracker'));
    $PAGE->set_heading(format_string($course->fullname));
    $PAGE->set_context($context);

    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('licensedemosubtitlestitle', 'videotracker'));
    echo html_writer::div(
        ...
        html_writer::tag('p', get_string('licensedemosubtitlesbody', 'videotracker'), ['class' => 'mb-3']) .
        html_writer::div(
            html_writer::div(
        ...
        html_writer::div(
            ...
            (
                $settingsurl instanceof moodle_url
                    ? ' ' . html_writer::link(
                        $settingsurl,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action !== '') {
    ...
    try {
        switch ($action) {
            ...
            case 'uploadsource':
                $sourcelang = optional_param('sourcelang', 'en', PARAM_ALPHANUMEXT);
                $upload = $_FILES['sourcevtt'] ?? null;
                if (empty($upload) || !is_array($upload)) {
                    throw new moodle_exception('subtitleerrornosourceupload', 'videotracker');
                }
                ...
                \mod_videotracker\local\subtitle_manager::import_source_vtt(
                    $videotracker,
                    $cm,
                    (string) $content,
                    $sourcelang
                );
                $message = get_string('subtitlesourceuploadok', 'videotracker');
                break;
...
$sourcegenerationreason = \mod_videotracker\local\subtitle_manager::get_source_generation_unavailable_reason();
$sourcelanguageoptions = \mod_videotracker\local\subtitle_manager::get_supported_translation_languages();
$translationlanguageoptions = $sourcelanguageoptions;

if ($sourceready && !empty($source->langcode)) {
    $sourcecode = \mod_videotracker\local\subtitle_manager::normalise_language_code((string) $source->langcode);
    unset($translationlanguageoptions[$sourcecode]);
}
...
$sourcehelpstring = ($sourcegenerationreason === '')
    ? 'subtitlesourcehelp'
    : 'subtitlesourceuploadfallbackhelp';
$sourcebody = html_writer::tag('p', get_string($sourcehelpstring, 'videotracker'), ['class' => 'mb-3']);
$sourcebody .= html_writer::div(
    html_writer::tag('strong', get_string('subtitlesourcestatus', 'videotracker')) . ': ' . $sourcebadge,
    'mb-3'
);
if ($sourcegenerationreason === '') {
    ...
    $sourcebody .= $OUTPUT->notification(
        get_string($sourcegenerationreason, 'videotracker'),
        \core\output\notification::NOTIFY_WARNING
    );

    $sourceoptionshtml = '';
    foreach ($sourcelanguageoptions as $code => $label) {
        $attributes = ['value' => $code];
        if ($code === 'en') {
            $attributes['selected'] = 'selected';
        }
        $sourceoptionshtml .= html_writer::tag('option', s($label), $attributes);
    }

    $uploadform = html_writer::start_tag('form', [
        'action' => $pageurl->out(false),
        'method' => 'post',
        'enctype' => 'multipart/form-data',
        'class' => 'mb-0 vt-subtitle-form vt-subtitle-upload-form',
    ]);
    $uploadform .= html_writer::empty_tag('input', [
        'type' => 'hidden',
        'name' => 'sesskey',
        'value' => sesskey(),
    ]);
    ...
    $uploadform .= html_writer::tag('label', get_string('subtitlesourceuploadfile', 'videotracker'), [
        'for' => 'videotracker-source-vtt',
        'class' => 'form-label',
    ]);
    $uploadform .= html_writer::empty_tag('input', [
        'type' => 'file',
        'name' => 'sourcevtt',
        'id' => 'videotracker-source-vtt',
        'accept' => '.vtt,text/vtt',
        'class' => 'form-control mb-3 vt-subtitle-file-control',
        'required' => 'required',
    ]);
    $uploadform .= html_writer::tag('label', get_string('subtitlesourcelanguage', 'videotracker'), [
        'for' => 'videotracker-source-language',
        'class' => 'form-label',
    ]);
    $uploadform .= html_writer::tag('select', $sourceoptionshtml, [
        'name' => 'sourcelang',
        'id' => 'videotracker-source-language',
        'class' => 'form-select mb-3 vt-subtitle-source-language',
    ]);
    $uploadbuttonlabel = $source
        ? get_string('subtitlesourceuploadreplace', 'videotracker')
        : get_string('subtitlesourceuploadbutton', 'videotracker');
    $uploadform .= html_writer::div(
        html_writer::tag(
            'button',
            $uploadbuttonlabel,
            [
                'type' => 'submit',
                'class' => 'btn btn-primary vt-subtitle-submit',
            ]
        ),
        'vt-subtitle-actions'
    );
    $uploadform .= html_writer::end_tag('form');

    $sourcebody .= $uploadform;
}

echo html_writer::div(
    html_writer::tag('h3', get_string('subtitlesourceheading', 'videotracker'), ['class' => 'h5']) . $sourcebody,
    'card card-body mb-4 vt-subtitle-card'
);

if ($sourceready) {
    $optionshtml = '';
    foreach ($translationlanguageoptions as $code => $label) {
    ...
    $translateform = html_writer::start_tag('form', [
        'action' => $pageurl->out(false),
        'method' => 'post',
        'class' => 'card card-body mb-4 vt-subtitle-card vt-subtitle-form',
    ]);
    ...
    $translateform .= html_writer::tag('select', $optionshtml, [
        ...
        'multiple' => 'multiple',
        'size' => '12',
        'class' => 'form-select mb-3 vt-subtitle-language-select',
    ]);
    $translateform .= html_writer::div(
        html_writer::tag('button', get_string('subtitletranslationqueue', 'videotracker'), [
            'type' => 'submit',
            'class' => 'btn btn-primary vt-subtitle-submit',
        ]),
        'vt-subtitle-actions'
